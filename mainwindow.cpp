...
void MainWindow::on_pushButton_compute_clicked()
{
    ...
    for (final_values[3] = -10; final_values[3] < 10.001; final_values[3] += 0.1)
         for (final_values[4] = -10; final_values[4] < 10.001; final_values[4] += 0.1)
         {
...
//             for (double t_sw = T / 20; t_sw < (T * 99 / 100); t_sw += T / 20)
             {
                 ...
                 en_turn_min = energy(control, t_sw, T);
