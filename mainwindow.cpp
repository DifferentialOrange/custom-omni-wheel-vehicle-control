...
void MainWindow::on_pushButton_compute_clicked()
{
    ...
    std::ofstream file("../custom-omni-wheel-vehicle-control/triangle.txt");

    for (int i = 0; i < N_symm_1.length(); i++) {
        double n_1 = (- N_symm_1[i] / sqrt(2) + N_symm_2[i] / sqrt(2)) / 9.8;
        double n_2 = (- N_symm_1[i] / sqrt(6) - N_symm_2[i] / sqrt(6) + 2 * N_symm_3[i] / sqrt(6)) / 9.8;
        file << n_1 << ' ' << n_2 << '\n';
    }
